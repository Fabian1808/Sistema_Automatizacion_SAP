...
from .sap_exceptions import SapError


class ComSapElement(SapElement):
...
class ComSapClient(ISapClient):
    ...
    def _wrap_element(self, path: str, com_element: Any) -> SapElement:
        return ComSapElement(path, com_element)
    ...
    def find_element(self, path: str, timeout: float = 10.0) -> SapElement:
        ...
        while time.monotonic() < limit:
            try:
                ...
                return self._wrap_element(path, com_el)
            except Exception as exc:
                last_error = exc
                time.sleep(0.5)
...
class SapClientFactory:
    ...
    @staticmethod
    def create(backend: str = "com", **kwargs) -> ISapClient:
        if backend == "com":
